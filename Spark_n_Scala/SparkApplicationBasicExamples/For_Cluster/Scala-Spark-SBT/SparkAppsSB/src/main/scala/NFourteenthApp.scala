...
object NFourteenthApp extends App {
  ...
  val tmpDepartments = broadcast(deptDF.as("departments"))
 
  empDF.join(broadcast(tmpDepartments), 
  $"dept_id" === $"emp_dept_id",  // join by employees.depID == departments.id 
  "inner").show()
}
